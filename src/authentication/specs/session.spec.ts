...
describe('Session', () => {
    describe('start', () => {
        it('should set dates', () => {
            ...
            expect(session.validUntil.valueOf()).toBeGreaterThan(
                session.startedAt.valueOf(),
            );
            expect(session.refreshableUntil.valueOf()).toBeGreaterThanOrEqual(
                session.validUntil.valueOf(),
            );
        });
    });
    ...
    describe('refresh', () => {
        ...
        it('should push validUntil by session length', () => {
            ...
            MockDate.set(date);
            session.refresh();
            const sessionEnd = moment(date)
                .add(SESSION_LENGTH, 'm')
                .toDate()
                .valueOf();
