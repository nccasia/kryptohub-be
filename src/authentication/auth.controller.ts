...
export class AuthController {
    ...
    @Post('login')
    async login(@Body() args: LoginRequest, @Res() res: Response) {
    ...
    async getWeb3LoginCode(@Query() req: Web3LoginCodeRequest) {
        ...
        return this.verificationService.generateEthVerificationCode(
            req.ethereumAddress,
        );
    }
    ...
    async web3Login(@Body() body: Web3LoginRequest, @Res() res: Response) {
        const signatureOk = await this.verificationService.verifySignedCode(
