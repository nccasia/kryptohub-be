...
describe('Verification module', () => {
    ...
    it('should generate a verification code and save it', async () => {
        expect(
            await verificationService.generateEthVerificationCode(DEAD_ADDRESS),
        ).toBeTruthy();
        expect(
            await verificationService.findEthCode(DEAD_ADDRESS),
        ).toBeTruthy();
    });

    it('Should not find a code', async () => {
        const code = await verificationService.generateEthVerificationCode(
            '0x00000',
        );
    ...
    it('Should use the code', async () => {
        ...
        expect(
            (await verificationRepo.findOne({identifier: DEAD_ADDRESS})).used,
        ).toBe(true);
    });
